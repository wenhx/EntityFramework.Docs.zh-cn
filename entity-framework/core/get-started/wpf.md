...
* 可通过多种方式配置 EF Core `DbContext`。 你可以从以下位置了解相关信息：[配置 DbContext](/ef/core/miscellaneous/configuring-dbcontext)。
...
> [!TIP]
> 了解使数据库和 EF Core 模型保持同步的不同方式：[管理数据库架构](/ef/core/managing-schemas)。
...
> [!NOTE]
> 代码使用对 `EnsureCreated()` 的调用在首次运行时生成数据库。 这对于演示是可接受的，但在生产应用中，应查看[迁移](/ef/core/managing-schemas/migrations/)来管理你的架构。 代码还会同步执行，因为它使用本地 SQLite 数据库。 对于通常涉及远程服务器的生产方案，请考虑使用 `Load` 和 `SaveChanges` 方法的异步版本。
...
了解有关[配置 DbContext](/ef/core/miscellaneous/configuring-dbcontext) 的详细信息。