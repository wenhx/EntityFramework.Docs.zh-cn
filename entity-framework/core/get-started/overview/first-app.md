...
* 运行以下命令：
  ...
  这会安装 [dotnet ef](xref:core/cli/dotnet) 和设计包，这是对项目运行命令所必需的。 `migrations` 命令为迁移搭建基架，以便为模型创建一组初始表。 `database update` 命令创建数据库并向其应用新的迁移。
...
* 在“包管理器控制台(PMC)”中，运行以下命令
  ...
  这会安装 [ EF Core 的 PMC 工具](xref:core/cli/powershell)。 `Add-Migration` 命令为迁移搭建基架，以便为模型创建一组初始表。 `Update-Database` 命令创建数据库并向其应用新的迁移。
