...
* `dotnet ef` 必须安装为全局或本地工具。 大多数开发人员会使用以下命令将 @no__t 0 作为全局工具安装：

  ``` console
  dotnet tool install --global dotnet-ef
  ```

  你还可以使用 @no__t 为本地工具。 若要将其用作本地工具，请使用[工具清单文件](https://github.com/dotnet/cli/issues/10288)还原项目的依赖项，将该项目声明为工具依赖项。
...
* 安装当前[.NET Core SDK](https://www.microsoft.com/net/download/core)。 即使有 Visual Studio 2017 的最新版本，也必须安装 SDK。
  ...
  这就是 ASP.NET Core 2.1 + 所需的全部，因为 `Microsoft.EntityFrameworkCore.Design` 包包含在[AspNetCore 元包](/aspnet/core/fundamentals/metapackage-app)中。

### <a name="ef-core-2x-not-aspnet-core"></a>EF Core 2.x （不 ASP.NET Core）

@No__t-0 命令包含在 .NET Core SDK 中，但要启用这些命令，必须安装 @no__t 1 包。
...
* 编辑项目文件，并添加 `Microsoft.EntityFrameworkCore.Tools.DotNet` 作为 @no__t 项。 指定最新的1.x 版本，例如：1.1.6. 请参阅本部分末尾的项目文件示例。

* 安装最新的 `Microsoft.EntityFrameworkCore.Design` 包版本，例如：
  ...
  不会向引用此项目的项目公开 @no__t 为-0 的包引用。 对于通常只在开发过程中使用的包，此限制特别有用。
...
* 该*项目*也称为*目标项目*，因为它是命令在其中添加或删除文件的位置。 默认情况下，当前目录中的项目是目标项目。 您可以通过使用<nobr>`--project`</nobr>选项指定其他项目作为目标项目。

* *启动项目*是工具生成和运行的项目。 这些工具必须在设计时执行应用程序代码，以获取有关项目的信息，例如数据库连接字符串和模型的配置。 默认情况下，当前目录中的项目是启动项目。 您可以通过使用<nobr>`--startup-project`</nobr>选项，将其他项目指定为启动项目。
...
CLI 工具适用于 .NET Core 项目和 .NET Framework 项目。 .NET Standard 类库中具有 EF Core 模型的应用可能没有 .NET Core 或 .NET Framework 项目。 例如，这适用于 Xamarin 和通用 Windows 平台应用。 在这种情况下，你可以创建一个仅供其使用的 .NET Core 控制台应用项目，作为工具的启动项目。 项目可以是不包含实际代码的虚拟项目 &mdash; 只需要为工具提供目标。
...
|                   | `--configuration <CONFIGURATION>` | 生成配置，例如： `Debug` 或 @no__t 为-1。                                                                                                                                                                                                   |
...
为数据库的 @no__t 和实体类型生成代码。 为了使此命令生成实体类型，数据库表必须具有主键。

参数：

| 参数       | 描述                                                                                                                                                                                                             |
|:---------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `<CONNECTION>` | 数据库的连接字符串。 对于 ASP.NET Core 2.x 项目，值可以是 *> 连接字符串的名称 = @no__t 1name*。 在这种情况下，该名称来自为项目设置的配置源。 |
...
|                 | `--context-dir <PATH>`                   | 要在其中放置 @no__t 0 类文件的目录。 路径相对于项目目录。 命名空间是从文件夹名称派生的。                                 |
| `-f`            | `--force`                                | 覆盖现有文件。                                                                                                                                                      |
