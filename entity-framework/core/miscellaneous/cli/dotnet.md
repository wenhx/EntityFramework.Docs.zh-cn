...
用于 Entity Framework Core 的命令行接口（CLI）工具执行设计时开发任务。 例如，他们基于现有数据库创建[迁移](/aspnet/core/data/ef-mvc/migrations?view=aspnetcore-2.0#introduction-to-migrations)、应用迁移和生成模型的代码。 命令是跨平台[dotnet](/dotnet/core/tools)命令的扩展，它是[.NET Core SDK](https://www.microsoft.com/net/core)的一部分。 这些工具适用于 .NET Core 项目。
...
* 安装最新`Microsoft.EntityFrameworkCore.Design`的包。
...
命令包含在 .NET Core SDK 中，但要启用这些命令，必须`Microsoft.EntityFrameworkCore.Design`安装包。 `dotnet ef`

* 安装当前[.NET Core SDK](https://www.microsoft.com/net/download/core)。 即使使用最新版本的 Visual Studio，也必须安装 SDK。

* 安装最新的`Microsoft.EntityFrameworkCore.Design`稳定包。
...
* 编辑项目文件，并添加`Microsoft.EntityFrameworkCore.Tools.DotNet` `DotNetCliToolReference`为项。 指定最新的1.x 版本，例如：1.1.6. 请参阅本部分末尾的项目文件示例。
...
* 安装最新版本的`Microsoft.EntityFrameworkCore.Design`包，例如：
  ...
  不会向引用`PrivateAssets="All"`此项目的项目公开包含的包引用。 对于通常只在开发过程中使用的包，此限制特别有用。
...
* 该*项目*也称为*目标项目*，因为它是命令在其中添加或删除文件的位置。 默认情况下，当前目录中的项目是目标项目。 可以使用<nobr>`--project`</nobr>选项指定其他项目作为目标项目。

* *启动项目*是工具生成和运行的项目。 这些工具必须在设计时执行应用程序代码，以获取有关项目的信息，例如数据库连接字符串和模型的配置。 默认情况下，当前目录中的项目是启动项目。 您可以使用<nobr>`--startup-project`</nobr>选项指定其他项目作为启动项目。
...
| 参数       | 描述                                                                                                                                                                                                             |
|:---------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `<CONNECTION>` | 数据库的连接字符串。 对于 ASP.NET Core 2.x 项目，值可以是*name =\<连接字符串 > 的名称*。 在这种情况下，该名称来自为项目设置的配置源。 |
...
|                 | `--context-dir <PATH>`                   | 要放`DbContext`入类文件的目录。 路径相对于项目目录。 命名空间是从文件夹名称派生的。                                 |
| `-f`            | `--force`                                | 覆盖现有文件。                                                                                                                                                      |
| `-o`            | `--output-dir <PATH>`                    | 要在其中放置实体类文件的目录。 路径相对于项目目录。                                                                                       |
|                 | <nobr>`--schema <SCHEMA_NAME>...`</nobr> | 要为其生成实体类型的表的架构。 若要指定多个架构`--schema` ，请重复上述操作。 如果省略此选项，则包括所有架构。          |
| `-t`            | `--table <TABLE_NAME>`...                | 要为其生成实体类型的表。 若要指定多个表`-t` ， `--table`请对每个表重复或。 如果省略此选项，则包括所有表。                |
