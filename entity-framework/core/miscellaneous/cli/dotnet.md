...
|                   | 选项                   | 说明                                              |
|:------------------|:-------------------------|:---------------------------------------------------------|
| <nobr>    </nobr> |  `--connection <CONNECTION>`        | 用于连接到数据库的连接字符串。 默认为或`AddDbContext` `OnConfiguring`中指定的一个。 |
...
|                 | `--context-namespace <NAMESPACE>`        | 要用于生成`DbContext`的类的命名空间。 注意：重`--namespace`写。                                 |
| `-f`            | `--force`                                | 覆盖现有文件。                                                                                                                                                      |
