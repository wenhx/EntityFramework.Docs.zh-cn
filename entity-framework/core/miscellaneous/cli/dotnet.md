...
`dotnet ef` 可以安装为全局或本地工具。 大多数开发人员更喜欢 `dotnet ef` 使用以下命令安装为全局工具：

```dotnetcli
dotnet tool install --global dotnet-ef
```

若要将其用作本地工具，请使用 [工具清单文件](/dotnet/core/tools/global-tools#install-a-local-tool)还原项目的依赖项，将该项目声明为工具依赖项。
