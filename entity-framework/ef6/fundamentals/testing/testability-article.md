...
<span data-ttu-id="29bb9-179">对于员工对象的 IRepository，代码可以执行以下操作。</span><span class="sxs-lookup"><span data-stu-id="29bb9-179">Given an IRepository of Employee objects, code can perform the following operations.</span></span>
...
<span data-ttu-id="29bb9-194">如果曾经使用过 ADO.NET 数据集完成任何工作，则已熟悉工作单元模式。</span><span class="sxs-lookup"><span data-stu-id="29bb9-194">If you’ve ever done any work with ADO.NET DataSets then you’ll already be familiar with the unit of work pattern.</span></span> <span data-ttu-id="29bb9-195">ADO.NET 数据集能够跟踪对 DataRow 对象的更新、删除和插入操作，并可以 (TableAdapter) 协调对数据库所做的所有更改。</span><span class="sxs-lookup"><span data-stu-id="29bb9-195">ADO.NET DataSets had the ability to track our updates, deletions, and insertion of DataRow objects and could (with the help of a TableAdapter) reconcile all our changes to a database.</span></span> <span data-ttu-id="29bb9-196">但是，DataSet 对象会为底层数据库的断开连接的子集建模。</span><span class="sxs-lookup"><span data-stu-id="29bb9-196">However, DataSet objects model a disconnected subset of the underlying database.</span></span> <span data-ttu-id="29bb9-197">工作单元模式的行为相同，但它适用于与数据访问代码隔离并不知道数据库的业务对象和域对象。</span><span class="sxs-lookup"><span data-stu-id="29bb9-197">The unit of work pattern exhibits the same behavior, but works with business objects and domain objects that are isolated from data access code and unaware of the database.</span></span>
...
## <a name="implementing-patterns-with-the-entity-framework"></a><span data-ttu-id="29bb9-221">用实体框架实现模式</span><span class="sxs-lookup"><span data-stu-id="29bb9-221">Implementing Patterns with the Entity Framework</span></span>
...
<span data-ttu-id="29bb9-234">**图 1**</span><span class="sxs-lookup"><span data-stu-id="29bb9-234">**Figure 1**</span></span>
...
<span data-ttu-id="29bb9-249">代码是否可测试？</span><span class="sxs-lookup"><span data-stu-id="29bb9-249">Is the code testable?</span></span> <span data-ttu-id="29bb9-250">至少需要两个测试来验证操作的行为。</span><span class="sxs-lookup"><span data-stu-id="29bb9-250">There are at least two tests we’d need to verify the action’s behavior.</span></span> <span data-ttu-id="29bb9-251">首先，我们想要验证操作是否返回正确的视图–一个简单的测试。</span><span class="sxs-lookup"><span data-stu-id="29bb9-251">First, we’d like to verify the action returns the correct view – an easy test.</span></span> <span data-ttu-id="29bb9-252">我们还希望编写一个测试来验证该操作是否可检索正确的雇员，我们想要执行此操作，而不执行代码查询数据库。</span><span class="sxs-lookup"><span data-stu-id="29bb9-252">We’d also want to write a test to verify the action retrieves the correct employee, and we’d like to do this without executing code to query the database.</span></span> <span data-ttu-id="29bb9-253">请记住，我们想要隔离受测代码。</span><span class="sxs-lookup"><span data-stu-id="29bb9-253">Remember we want to isolate the code under test.</span></span> <span data-ttu-id="29bb9-254">隔离将确保测试不会因数据访问代码或数据库配置中的错误而失败。</span><span class="sxs-lookup"><span data-stu-id="29bb9-254">Isolation will ensure the test doesn’t fail because of a bug in the data access code or database configuration.</span></span> <span data-ttu-id="29bb9-255">如果测试失败，我们将了解控制器逻辑中的 bug，而不是在某个较低级别的系统组件中。</span><span class="sxs-lookup"><span data-stu-id="29bb9-255">If the test fails, we will know we have a bug in the controller logic, and not in some lower level system component.</span></span>
...
### <a name="the-test-doubles"></a><span data-ttu-id="29bb9-269">测试双精度</span><span class="sxs-lookup"><span data-stu-id="29bb9-269">The Test Doubles</span></span>
...
<span data-ttu-id="29bb9-277">请注意，伪工作单元公开了提交的属性。</span><span class="sxs-lookup"><span data-stu-id="29bb9-277">Notice the fake unit of work exposes a Commited property.</span></span> <span data-ttu-id="29bb9-278">有时将功能添加到可简化测试的虚设类中。</span><span class="sxs-lookup"><span data-stu-id="29bb9-278">It’s sometimes useful to add features to a fake class that facilitate testing.</span></span> <span data-ttu-id="29bb9-279">在这种情况下，如果代码通过检查确认属性来提交工作单元，则很容易观察。</span><span class="sxs-lookup"><span data-stu-id="29bb9-279">In this case it is easy to observe if code commits a unit of work by checking the Commited property.</span></span>
...
### <a name="projection"></a><span data-ttu-id="29bb9-328">投影</span><span class="sxs-lookup"><span data-stu-id="29bb9-328">Projection</span></span>
...
### <a name="lazy-loading"></a><span data-ttu-id="29bb9-368">延迟加载</span><span class="sxs-lookup"><span data-stu-id="29bb9-368">Lazy Loading</span></span>
...
<span data-ttu-id="29bb9-385">我们仍然可以说，Employee 实体主要是持久性未知的。</span><span class="sxs-lookup"><span data-stu-id="29bb9-385">We can still say the Employee entity is mostly persistence ignorant.</span></span> <span data-ttu-id="29bb9-386">唯一的要求是使用虚拟成员，这不会影响代码的可测试性。</span><span class="sxs-lookup"><span data-stu-id="29bb9-386">The only requirement is to use virtual members and this does not impact the testability of the code.</span></span> <span data-ttu-id="29bb9-387">不需要从任何特殊基类派生，甚至可以使用专用于延迟加载的特殊集合。</span><span class="sxs-lookup"><span data-stu-id="29bb9-387">We don’t need to derive from any special base class, or even use a special collection dedicated to lazy loading.</span></span> <span data-ttu-id="29bb9-388">如代码所示，任何实现 ICollection &lt; T &gt; 的类都可用于保存相关实体。</span><span class="sxs-lookup"><span data-stu-id="29bb9-388">As the code demonstrates, any class implementing ICollection&lt;T&gt; is available to hold related entities.</span></span>

<span data-ttu-id="29bb9-389">还需要在我们的工作单元内进行一个小的更改。</span><span class="sxs-lookup"><span data-stu-id="29bb9-389">There is also one minor change we need to make inside our unit of work.</span></span> <span data-ttu-id="29bb9-390">当直接使用 ObjectContext 对象时，延迟加载默认情况下处于 *关闭状态* 。</span><span class="sxs-lookup"><span data-stu-id="29bb9-390">Lazy loading is *off* by default when working directly with an ObjectContext object.</span></span> <span data-ttu-id="29bb9-391">可以在 ContextOptions 属性上设置一个属性来启用延迟加载，如果我们想要在任何位置启用延迟加载，则可以在实际工作单元内设置此属性。</span><span class="sxs-lookup"><span data-stu-id="29bb9-391">There is a property we can set on the ContextOptions property to enable deferred loading, and we can set this property inside our real unit of work if we want to enable lazy loading everywhere.</span></span>
...
<span data-ttu-id="29bb9-392">启用隐式延迟加载后，应用程序代码可以使用员工和员工的相关时间表，同时丝毫不知道 EF 加载额外数据所需的工作。</span><span class="sxs-lookup"><span data-stu-id="29bb9-392">With implicit lazy loading enabled, application code can use an employee and the employee’s associated time cards while remaining blissfully unaware of the work required for EF to load the extra data.</span></span>
...
<span data-ttu-id="29bb9-393">延迟加载使得应用程序代码更易于编写，而对于代理的神奇，代码会保持完全可测试性。</span><span class="sxs-lookup"><span data-stu-id="29bb9-393">Lazy loading makes the application code easier to write, and with the proxy magic the code remains completely testable.</span></span> <span data-ttu-id="29bb9-394">工作单元的内存中 fakes 只需在测试过程中需要时使用关联数据预加载虚设实体。</span><span class="sxs-lookup"><span data-stu-id="29bb9-394">In-memory fakes of the unit of work can simply preload fake entities with associated data when needed during a test.</span></span>

<span data-ttu-id="29bb9-395">此时，我们将使用 IObjectSet T 来介绍如何生成存储库 &lt; &gt; ，并查看抽象以隐藏持久性框架的所有标志。</span><span class="sxs-lookup"><span data-stu-id="29bb9-395">At this point we’ll turn our attention from building repositories using IObjectSet&lt;T&gt; and look at abstractions to hide all signs of the persistence framework.</span></span>

## <a name="custom-repositories"></a><span data-ttu-id="29bb9-396">自定义存储库</span><span class="sxs-lookup"><span data-stu-id="29bb9-396">Custom Repositories</span></span>

<span data-ttu-id="29bb9-397">当我们在本文中第一次介绍工作单元设计模式时，我们提供了一些示例代码，说明工作单元的外观。</span><span class="sxs-lookup"><span data-stu-id="29bb9-397">When we first presented the unit of work design pattern in this article we provided some sample code for what the unit of work might look like.</span></span> <span data-ttu-id="29bb9-398">让我们使用我们一直在使用的员工和员工时间表方案来重新展示这一思路。</span><span class="sxs-lookup"><span data-stu-id="29bb9-398">Let’s re-present this original idea using the employee and employee time card scenario we’ve been working with.</span></span>
...
<span data-ttu-id="29bb9-399">此工作单元和我们在上一节中创建的工作单元之间的主要区别在于，此工作单元不会使用 EF4 框架中的任何抽象 (没有) 的 IObjectSet &lt; &gt; 。</span><span class="sxs-lookup"><span data-stu-id="29bb9-399">The primary difference between this unit of work and the unit of work we created in the last section is how this unit of work does not use any abstractions from the EF4 framework (there is no IObjectSet&lt;T&gt;).</span></span> <span data-ttu-id="29bb9-400">IObjectSet &lt; T &gt; 适用于存储库接口，但它公开的 API 可能并不完全符合应用程序的需求。</span><span class="sxs-lookup"><span data-stu-id="29bb9-400">IObjectSet&lt;T&gt; works well as a repository interface, but the API it exposes might not perfectly align with our application’s needs.</span></span> <span data-ttu-id="29bb9-401">在即将推出的方法中，我们将使用自定义 IRepository T 抽象来表示存储库 &lt; &gt; 。</span><span class="sxs-lookup"><span data-stu-id="29bb9-401">In this upcoming approach we will represent repositories using a custom IRepository&lt;T&gt; abstraction.</span></span>

<span data-ttu-id="29bb9-402">许多遵循测试驱动设计、行为驱动设计和域驱动方法设计的开发人员 &lt; 出于多种原因而倾向于使用 IRepository T &gt; 方法。</span><span class="sxs-lookup"><span data-stu-id="29bb9-402">Many developers who follow test-driven design, behavior-driven design, and domain driven methodologies design prefer the IRepository&lt;T&gt; approach for several reasons.</span></span> <span data-ttu-id="29bb9-403">首先，IRepository &lt; T &gt; 接口表示 "反损坏" 层。</span><span class="sxs-lookup"><span data-stu-id="29bb9-403">First, the IRepository&lt;T&gt; interface represents an “anti-corruption” layer.</span></span> <span data-ttu-id="29bb9-404">正如 Eric Evans 在他的域驱动的设计书中所述，抗损坏层会使你的域代码远离基础结构 Api，如永久性 API。</span><span class="sxs-lookup"><span data-stu-id="29bb9-404">As described by Eric Evans in his Domain Driven Design book an anti-corruption layer keeps your domain code away from infrastructure APIs, like a persistence API.</span></span> <span data-ttu-id="29bb9-405">其次，开发人员可以将方法构建到存储库中，以满足在) 编写测试时发现的应用程序 (的确切需求。</span><span class="sxs-lookup"><span data-stu-id="29bb9-405">Secondly, developers can build methods into the repository that meet the exact needs of an application (as discovered while writing tests).</span></span> <span data-ttu-id="29bb9-406">例如，我们可能经常需要使用 ID 值查找单个实体，因此可以将 FindById 方法添加到存储库接口。</span><span class="sxs-lookup"><span data-stu-id="29bb9-406">For example, we might frequently need to locate a single entity using an ID value, so we can add a FindById method to the repository interface.</span></span><span data-ttu-id="29bb9-407">IRepository &lt; T &gt; 定义将如下所示。</span><span class="sxs-lookup"><span data-stu-id="29bb9-407">  Our IRepository&lt;T&gt; definition will look like the following.</span></span>
...
<span data-ttu-id="29bb9-408">请注意，我们将返回使用 IQueryable &lt; T &gt; 接口公开实体集合。</span><span class="sxs-lookup"><span data-stu-id="29bb9-408">Notice we’ll drop back to using an IQueryable&lt;T&gt; interface to expose entity collections.</span></span> <span data-ttu-id="29bb9-409">IQueryable &lt; T &gt; 允许 LINQ 表达式树流入 EF4 提供程序，并为提供程序提供查询的整体视图。</span><span class="sxs-lookup"><span data-stu-id="29bb9-409">IQueryable&lt;T&gt; allows LINQ expression trees to flow into the EF4 provider and give the provider a holistic view of the query.</span></span> <span data-ttu-id="29bb9-410">第二个选项是返回 IEnumerable &lt; T &gt; ，这意味着 EF4 LINQ 提供程序将只能看到在存储库内生成的表达式。</span><span class="sxs-lookup"><span data-stu-id="29bb9-410">A second option would be to return IEnumerable&lt;T&gt;, which means the EF4 LINQ provider will only see the expressions built inside of the repository.</span></span> <span data-ttu-id="29bb9-411">不会将存储库外部完成的任何分组、排序和投影组合到发送到数据库的 SQL 命令中，这可能会影响性能。</span><span class="sxs-lookup"><span data-stu-id="29bb9-411">Any grouping, ordering, and projection done outside of the repository will not be composed into the SQL command sent to the database, which can hurt performance.</span></span> <span data-ttu-id="29bb9-412">另一方面，只返回 IEnumerable T 结果的存储库 &lt; &gt; 永远不会让你使用新的 SQL 命令。</span><span class="sxs-lookup"><span data-stu-id="29bb9-412">On the other hand, a repository returning only IEnumerable&lt;T&gt; results will never surprise you with a new SQL command.</span></span> <span data-ttu-id="29bb9-413">这两种方法都适用，并且这两种方法保持可测试性。</span><span class="sxs-lookup"><span data-stu-id="29bb9-413">Both approaches will work, and both approaches remain testable.</span></span>

<span data-ttu-id="29bb9-414">&lt; &gt; 使用泛型和 EF4 ObjectContext API 提供 IRepository T 接口的单个实现非常简单。</span><span class="sxs-lookup"><span data-stu-id="29bb9-414">It’s straightforward to provide a single implementation of the IRepository&lt;T&gt; interface using generics and the EF4 ObjectContext API.</span></span>
...
<span data-ttu-id="29bb9-415">IRepository &lt; T &gt; 方法为我们的查询提供了一些额外的控制，因为客户端必须调用方法来获取实体。</span><span class="sxs-lookup"><span data-stu-id="29bb9-415">The IRepository&lt;T&gt; approach gives us some additional control over our queries because a client has to invoke a method to get to an entity.</span></span> <span data-ttu-id="29bb9-416">在方法中，我们可以提供额外的检查和 LINQ 运算符来强制应用程序约束。</span><span class="sxs-lookup"><span data-stu-id="29bb9-416">Inside the method we could provide additional checks and LINQ operators to enforce application constraints.</span></span> <span data-ttu-id="29bb9-417">请注意，接口对泛型类型参数具有两个约束。</span><span class="sxs-lookup"><span data-stu-id="29bb9-417">Notice the interface has two constraints on the generic type parameter.</span></span> <span data-ttu-id="29bb9-418">第一个约束是 ObjectSet T 所需的类缺点 &lt; 破坏 &gt; ，第二个约束强制我们的实体实现 IEntity –为应用程序创建的抽象。</span><span class="sxs-lookup"><span data-stu-id="29bb9-418">The first constraint is the class cons taint required by ObjectSet&lt;T&gt;, and the second constraint forces our entities to implement IEntity – an abstraction created for the application.</span></span> <span data-ttu-id="29bb9-419">IEntity 接口强制实体具有可读 Id 属性，然后可以在 FindById 方法中使用此属性。</span><span class="sxs-lookup"><span data-stu-id="29bb9-419">The IEntity interface forces entities to have a readable Id property, and we can then use this property in the FindById method.</span></span> <span data-ttu-id="29bb9-420">IEntity 是用下面的代码定义的。</span><span class="sxs-lookup"><span data-stu-id="29bb9-420">IEntity is defined with the following code.</span></span>
...
<span data-ttu-id="29bb9-421">IEntity 可能被视为一小部分持久性无感知，因为需要实体才能实现此接口。</span><span class="sxs-lookup"><span data-stu-id="29bb9-421">IEntity could be considered a small violation of persistence ignorance since our entities are required to implement this interface.</span></span> <span data-ttu-id="29bb9-422">请记住持久性无感知是关于权衡的，许多 FindById 功能将超过接口施加的约束。</span><span class="sxs-lookup"><span data-stu-id="29bb9-422">Remember persistence ignorance is about tradeoffs, and for many the FindById functionality will outweigh the constraint imposed by the interface.</span></span> <span data-ttu-id="29bb9-423">接口对可测试性没有影响。</span><span class="sxs-lookup"><span data-stu-id="29bb9-423">The interface has no impact on testability.</span></span>

<span data-ttu-id="29bb9-424">实例化实时 IRepository &lt; T &gt; 需要 EF4 ObjectContext，因此具体的工作单元实现应管理实例化。</span><span class="sxs-lookup"><span data-stu-id="29bb9-424">Instantiating a live IRepository&lt;T&gt; requires an EF4 ObjectContext, so a concrete unit of work implementation should manage the instantiation.</span></span>
...
### <a name="using-the-custom-repository"></a><span data-ttu-id="29bb9-425">使用自定义存储库</span><span class="sxs-lookup"><span data-stu-id="29bb9-425">Using the Custom Repository</span></span>

<span data-ttu-id="29bb9-426">使用自定义存储库并不大不同于使用基于 IObjectSet T 的存储库 &lt; &gt; 。</span><span class="sxs-lookup"><span data-stu-id="29bb9-426">Using our custom repository is not significantly different from using the repository based on IObjectSet&lt;T&gt;.</span></span> <span data-ttu-id="29bb9-427">我们首先需要调用一个存储库方法来获取 IQueryable T 引用，而不是直接对属性应用 LINQ 运算符 &lt; &gt; 。</span><span class="sxs-lookup"><span data-stu-id="29bb9-427">Instead of applying LINQ operators directly to a property, we’ll first need to invoke one the repository’s methods to grab an IQueryable&lt;T&gt; reference.</span></span>
...
<span data-ttu-id="29bb9-428">请注意，我们先前实现的自定义 Include 运算符无需更改即可运行。</span><span class="sxs-lookup"><span data-stu-id="29bb9-428">Notice the custom Include operator we implemented previously will work without change.</span></span> <span data-ttu-id="29bb9-429">存储库的 FindById 方法从尝试检索单个实体的操作中删除重复逻辑。</span><span class="sxs-lookup"><span data-stu-id="29bb9-429">The repository’s FindById method removes duplicated logic from actions trying to retrieve a single entity.</span></span>
...
<span data-ttu-id="29bb9-430">我们所检查的两种方法的可测试性并没有显著的差异。</span><span class="sxs-lookup"><span data-stu-id="29bb9-430">There is no significant difference in the testability of the two approaches we’ve examined.</span></span> <span data-ttu-id="29bb9-431">我们可以 &lt; &gt; 通过构建由 HashSet 员工支持的具体类（ &lt; &gt; 就像我们在上一节中所做的那样），提供 IRepository T 的伪实现。</span><span class="sxs-lookup"><span data-stu-id="29bb9-431">We could provide fake implementations of IRepository&lt;T&gt; by building concrete classes backed by HashSet&lt;Employee&gt; - just like what we did in the last section.</span></span> <span data-ttu-id="29bb9-432">但是，某些开发人员更喜欢使用 mock 对象和模拟对象框架，而不是生成 fakes。</span><span class="sxs-lookup"><span data-stu-id="29bb9-432">However, some developers prefer to use mock objects and mock object frameworks instead of building fakes.</span></span> <span data-ttu-id="29bb9-433">我们将在下一节中介绍如何使用模拟来测试实现并讨论模拟和 fakes 之间的差异。</span><span class="sxs-lookup"><span data-stu-id="29bb9-433">We’ll look at using mocks to test our implementation and discuss the differences between mocks and fakes in the next section.</span></span>

### <a name="testing-with-mocks"></a><span data-ttu-id="29bb9-434">用模拟进行测试</span><span class="sxs-lookup"><span data-stu-id="29bb9-434">Testing with Mocks</span></span>

<span data-ttu-id="29bb9-435">可以使用不同的方法来构建 Fowler 调用 "测试双精度" 的情况。</span><span class="sxs-lookup"><span data-stu-id="29bb9-435">There are different approaches to building what Martin Fowler calls a “test double”.</span></span> <span data-ttu-id="29bb9-436">测试双 (如电影 stunt 双) 是在测试期间生成的、用于实际生产对象的对象。</span><span class="sxs-lookup"><span data-stu-id="29bb9-436">A test double (like a movie stunt double) is an object you build to “stand in” for real, production objects during tests.</span></span> <span data-ttu-id="29bb9-437">我们创建的内存中存储库是与 SQL Server 通信的存储库的测试。</span><span class="sxs-lookup"><span data-stu-id="29bb9-437">The in-memory repositories we created are test doubles for the repositories that talk to SQL Server.</span></span> <span data-ttu-id="29bb9-438">我们已了解如何在单元测试过程中使用这些测试双线来隔离代码并使测试运行速度更快。</span><span class="sxs-lookup"><span data-stu-id="29bb9-438">We’ve seen how to use these test-doubles during the unit tests to isolate code and keep tests running fast.</span></span>

<span data-ttu-id="29bb9-439">我们生成的测试翻倍，实现了真实的工作实现。</span><span class="sxs-lookup"><span data-stu-id="29bb9-439">The test doubles we’ve built have real, working implementations.</span></span> <span data-ttu-id="29bb9-440">在幕后，每个对象都存储一个具体的对象集合，并将在测试过程中操作存储库时，在此集合中添加和移除对象。</span><span class="sxs-lookup"><span data-stu-id="29bb9-440">Behind the scenes each one stores a concrete collection of objects, and they will add and remove objects from this collection as we manipulate the repository during a test.</span></span> <span data-ttu-id="29bb9-441">一些开发人员喜欢以这种方式生成测试，这种方法包括真实的代码和工作实现。</span><span class="sxs-lookup"><span data-stu-id="29bb9-441">Some developers like to build their test doubles this way – with real code and working implementations.</span></span><span data-ttu-id="29bb9-442">这些测试是 *fakes*的。</span><span class="sxs-lookup"><span data-stu-id="29bb9-442">  These test doubles are what we call *fakes*.</span></span> <span data-ttu-id="29bb9-443">它们具有有效的实现，但并不真正足以用于生产。</span><span class="sxs-lookup"><span data-stu-id="29bb9-443">They have working implementations, but they aren’t real enough for production use.</span></span> <span data-ttu-id="29bb9-444">伪存储库实际上不会写入数据库。</span><span class="sxs-lookup"><span data-stu-id="29bb9-444">The fake repository doesn’t actually write to the database.</span></span> <span data-ttu-id="29bb9-445">伪 SMTP 服务器实际上不会通过网络发送电子邮件。</span><span class="sxs-lookup"><span data-stu-id="29bb9-445">The fake SMTP server doesn’t actually send an email message over the network.</span></span>

### <a name="mocks-versus-fakes"></a><span data-ttu-id="29bb9-446">模拟与 Fakes</span><span class="sxs-lookup"><span data-stu-id="29bb9-446">Mocks versus Fakes</span></span>

<span data-ttu-id="29bb9-447">还有另一种类型的测试 double 称为 *模拟*。</span><span class="sxs-lookup"><span data-stu-id="29bb9-447">There is another type of test double known as a *mock*.</span></span> <span data-ttu-id="29bb9-448">尽管 fakes 具有有效的实现，但模拟没有实现。</span><span class="sxs-lookup"><span data-stu-id="29bb9-448">While fakes have working implementations, mocks come with no implementation.</span></span> <span data-ttu-id="29bb9-449">使用 mock 对象框架的帮助，我们在运行时构造这些模拟对象并将其用作测试双精度型。</span><span class="sxs-lookup"><span data-stu-id="29bb9-449">With the help of a mock object framework we construct these mock objects at run time and use them as test doubles.</span></span> <span data-ttu-id="29bb9-450">在本部分中，我们将使用开源模拟 framework Moq。</span><span class="sxs-lookup"><span data-stu-id="29bb9-450">In this section we’ll be using the open source mocking framework Moq.</span></span> <span data-ttu-id="29bb9-451">下面是一个简单的示例，说明如何使用 Moq 为员工存储库动态创建测试 double。</span><span class="sxs-lookup"><span data-stu-id="29bb9-451">Here is a simple example of using Moq to dynamically create a test double for an employee repository.</span></span>
...
<span data-ttu-id="29bb9-452">我们要求 Moq IRepository &lt; 员工 &gt; 实现，并且它会动态生成一个。</span><span class="sxs-lookup"><span data-stu-id="29bb9-452">We ask Moq for an IRepository&lt;Employee&gt; implementation and it builds one dynamically.</span></span> <span data-ttu-id="29bb9-453">&lt; &gt; 通过访问 Mock T 对象的对象属性，我们可以访问实现 IRepository 员工的对象 &lt; &gt; 。</span><span class="sxs-lookup"><span data-stu-id="29bb9-453">We can get to the object implementing IRepository&lt;Employee&gt; by accessing the Object property of the Mock&lt;T&gt; object.</span></span> <span data-ttu-id="29bb9-454">我们可以将此内部对象传递到我们的控制器中，并不知道这是测试 double 还是真正的存储库。</span><span class="sxs-lookup"><span data-stu-id="29bb9-454">It is this inner object we can pass into our controllers, and they won’t know if this is a test double or the real repository.</span></span> <span data-ttu-id="29bb9-455">可以调用对象上的方法，就像我们使用实际实现调用对象上的方法一样。</span><span class="sxs-lookup"><span data-stu-id="29bb9-455">We can invoke methods on the object just like we would invoke methods on an object with a real implementation.</span></span>

<span data-ttu-id="29bb9-456">当我们调用 Add 方法时，你必须知道 mock 存储库将执行的操作。</span><span class="sxs-lookup"><span data-stu-id="29bb9-456">You must be wondering what the mock repository will do when we invoke the Add method.</span></span> <span data-ttu-id="29bb9-457">由于 mock 对象尚无实现，因此不执行任何操作。</span><span class="sxs-lookup"><span data-stu-id="29bb9-457">Since there is no implementation behind the mock object, Add does nothing.</span></span> <span data-ttu-id="29bb9-458">幕后没有具体的集合，就像我们编写的 fakes 一样，因此放弃了该员工。</span><span class="sxs-lookup"><span data-stu-id="29bb9-458">There is no concrete collection behind the scenes like we had with the fakes we wrote, so the employee is discarded.</span></span> <span data-ttu-id="29bb9-459">FindById 的返回值怎么样？</span><span class="sxs-lookup"><span data-stu-id="29bb9-459">What about the return value of FindById?</span></span> <span data-ttu-id="29bb9-460">在这种情况下，mock 对象执行的唯一操作就是返回默认值。</span><span class="sxs-lookup"><span data-stu-id="29bb9-460">In this case the mock object does the only thing it can do, which is return a default value.</span></span> <span data-ttu-id="29bb9-461">由于我们将返回雇员)  (引用类型，因此返回值为 null 值。</span><span class="sxs-lookup"><span data-stu-id="29bb9-461">Since we are returning a reference type (an Employee), the return value is a null value.</span></span>

<span data-ttu-id="29bb9-462">模拟可能会毫无意义。但是，我们还没有谈到模拟的两个功能。</span><span class="sxs-lookup"><span data-stu-id="29bb9-462">Mocks might sound worthless; however, there are two more features of mocks we haven’t talked about.</span></span> <span data-ttu-id="29bb9-463">首先，Moq 框架记录模拟对象上发出的所有调用。</span><span class="sxs-lookup"><span data-stu-id="29bb9-463">First, the Moq framework records all the calls made on the mock object.</span></span> <span data-ttu-id="29bb9-464">稍后在代码中，我们可以询问 Moq 是否有人调用了 Add 方法，或者有人调用了 FindById 方法。</span><span class="sxs-lookup"><span data-stu-id="29bb9-464">Later in the code we can ask Moq if anyone invoked the Add method, or if anyone invoked the FindById method.</span></span> <span data-ttu-id="29bb9-465">稍后我们将介绍如何在测试中使用此 "黑色框" 录制功能。</span><span class="sxs-lookup"><span data-stu-id="29bb9-465">We’ll see later how we can use this “black box” recording feature in tests.</span></span>

<span data-ttu-id="29bb9-466">第二大功能 *是，我们*可以使用 Moq 来为模拟对象编程。</span><span class="sxs-lookup"><span data-stu-id="29bb9-466">The second great feature is how we can use Moq to program a mock object with *expectations*.</span></span> <span data-ttu-id="29bb9-467">预期会告诉 mock 对象如何响应任何给定的交互。</span><span class="sxs-lookup"><span data-stu-id="29bb9-467">An expectation tells the mock object how to respond to any given interaction.</span></span> <span data-ttu-id="29bb9-468">例如，我们可以将预期计划为模拟，并告诉它在用户调用 FindById 时返回 employee 对象。</span><span class="sxs-lookup"><span data-stu-id="29bb9-468">For example, we can program an expectation into our mock and tell it to return an employee object when someone invokes FindById.</span></span> <span data-ttu-id="29bb9-469">Moq 框架使用安装 API 和 lambda 表达式来对这些预期进行编程。</span><span class="sxs-lookup"><span data-stu-id="29bb9-469">The Moq framework uses a Setup API and lambda expressions to program these expectations.</span></span>
...
<span data-ttu-id="29bb9-470">在此示例中，我们要求 Moq 动态构建一个存储库，然后对存储库进行计划。</span><span class="sxs-lookup"><span data-stu-id="29bb9-470">In this sample we ask Moq to dynamically build a repository, and then we program the repository with an expectation.</span></span> <span data-ttu-id="29bb9-471">当有人调用 FindById 方法传递值5时，预期会告知 mock 对象返回 Id 值为5的新 employee 对象。</span><span class="sxs-lookup"><span data-stu-id="29bb9-471">The expectation tells the mock object to return a new employee object with an Id value of 5 when someone invokes the FindById method passing a value of 5.</span></span> <span data-ttu-id="29bb9-472">此测试通过，我们不需要生成完整的实现来实现虚假 IRepository &lt; t &gt; 。</span><span class="sxs-lookup"><span data-stu-id="29bb9-472">This test passes, and we didn’t need to build a full implementation to fake IRepository&lt;T&gt;.</span></span>

<span data-ttu-id="29bb9-473">让我们重新访问之前编写的测试，并对其进行返工，使用模拟而不是 fakes。</span><span class="sxs-lookup"><span data-stu-id="29bb9-473">Let’s revisit the tests we wrote earlier and rework them to use mocks instead of fakes.</span></span> <span data-ttu-id="29bb9-474">就像以前一样，我们将使用基类来设置控制器所有测试所需的基础结构的公共部分。</span><span class="sxs-lookup"><span data-stu-id="29bb9-474">Just like before, we’ll use a base class to setup the common pieces of infrastructure we need for all of the controller’s tests.</span></span>
...
<span data-ttu-id="29bb9-475">安装程序代码的基本保持不变。</span><span class="sxs-lookup"><span data-stu-id="29bb9-475">The setup code remains mostly the same.</span></span> <span data-ttu-id="29bb9-476">我们将使用 Moq 来构造 mock 对象，而不是使用 fakes。</span><span class="sxs-lookup"><span data-stu-id="29bb9-476">Instead of using fakes, we’ll use Moq to construct mock objects.</span></span> <span data-ttu-id="29bb9-477">当代码调用 Employees 属性时，该基类将排列模拟工作单元以返回 mock 存储库。</span><span class="sxs-lookup"><span data-stu-id="29bb9-477">The base class arranges for the mock unit of work to return a mock repository when code invokes the Employees property.</span></span> <span data-ttu-id="29bb9-478">模拟设置的其余部分将在专用于每个特定方案的测试装置内进行。</span><span class="sxs-lookup"><span data-stu-id="29bb9-478">The rest of the mock setup will take place inside the test fixtures dedicated to each specific scenario.</span></span> <span data-ttu-id="29bb9-479">例如，当操作调用 mock 存储库的 FindAll 方法时，用于索引操作的测试装置将设置 mock 存储库以返回员工列表。</span><span class="sxs-lookup"><span data-stu-id="29bb9-479">For example, the test fixture for the Index action will setup the mock repository to return a list of employees when the action invokes the FindAll method of the mock repository.</span></span>
...
<span data-ttu-id="29bb9-480">除了预期之外，我们的测试看起来类似于以前的测试。</span><span class="sxs-lookup"><span data-stu-id="29bb9-480">Except for the expectations, our tests look similar to the tests we had before.</span></span> <span data-ttu-id="29bb9-481">然而，使用模拟框架的录制功能，我们可以从不同角度进行测试。</span><span class="sxs-lookup"><span data-stu-id="29bb9-481">However, with the recording ability of a mock framework we can approach testing from a different angle.</span></span> <span data-ttu-id="29bb9-482">在下一部分中，我们将介绍这个新的透视。</span><span class="sxs-lookup"><span data-stu-id="29bb9-482">We’ll look at this new perspective in the next section.</span></span>

### <a name="state-versus-interaction-testing"></a><span data-ttu-id="29bb9-483">状态与交互测试</span><span class="sxs-lookup"><span data-stu-id="29bb9-483">State versus Interaction Testing</span></span>

<span data-ttu-id="29bb9-484">你可以使用不同的技术来测试具有 mock 对象的软件。</span><span class="sxs-lookup"><span data-stu-id="29bb9-484">There are different techniques you can use to test software with mock objects.</span></span> <span data-ttu-id="29bb9-485">一种方法是使用基于状态的测试，这是我们目前在本文中所执行的操作。</span><span class="sxs-lookup"><span data-stu-id="29bb9-485">One approach is to use state based testing, which is what we have done in this paper so far.</span></span> <span data-ttu-id="29bb9-486">基于状态的测试对软件的状态进行断言。</span><span class="sxs-lookup"><span data-stu-id="29bb9-486">State based testing makes assertions about the state of the software.</span></span> <span data-ttu-id="29bb9-487">在最后一次测试中，我们在控制器上调用了操作方法，并对它应生成的模型作出了一个断言。</span><span class="sxs-lookup"><span data-stu-id="29bb9-487">In the last test we invoked an action method on the controller and made an assertion about the model it should build.</span></span> <span data-ttu-id="29bb9-488">下面是测试状态的一些其他示例：</span><span class="sxs-lookup"><span data-stu-id="29bb9-488">Here are some other examples of testing state:</span></span>

-   <span data-ttu-id="29bb9-489">创建 "创建" 后，验证存储库是否包含新的 employee 对象。</span><span class="sxs-lookup"><span data-stu-id="29bb9-489">Verify the repository contains the new employee object after Create executes.</span></span>
-   <span data-ttu-id="29bb9-490">验证该模型在执行 Index 后是否包含所有员工的列表。</span><span class="sxs-lookup"><span data-stu-id="29bb9-490">Verify the model holds a list of all employees after Index executes.</span></span>
-   <span data-ttu-id="29bb9-491">验证 "删除" 执行后存储库不包含给定的员工。</span><span class="sxs-lookup"><span data-stu-id="29bb9-491">Verify the repository does not contain a given employee after Delete executes.</span></span>

<span data-ttu-id="29bb9-492">模拟对象的另一种方法是验证 *交互*。</span><span class="sxs-lookup"><span data-stu-id="29bb9-492">Another approach you’ll see with mock objects is to verify *interactions*.</span></span> <span data-ttu-id="29bb9-493">虽然基于状态的测试对对象的状态进行断言，但基于交互的测试会使断言与对象交互的方式有关。</span><span class="sxs-lookup"><span data-stu-id="29bb9-493">While state based testing makes assertions about the state of objects, interaction based testing makes assertions about how objects interact.</span></span> <span data-ttu-id="29bb9-494">例如：</span><span class="sxs-lookup"><span data-stu-id="29bb9-494">For example:</span></span>

-   <span data-ttu-id="29bb9-495">验证在创建执行时控制器是否调用存储库的 Add 方法。</span><span class="sxs-lookup"><span data-stu-id="29bb9-495">Verify the controller invokes the repository’s Add method when Create executes.</span></span>
-   <span data-ttu-id="29bb9-496">当执行索引时，验证控制器是否调用存储库的 FindAll 方法。</span><span class="sxs-lookup"><span data-stu-id="29bb9-496">Verify the controller invokes the repository’s FindAll method when Index executes.</span></span>
-   <span data-ttu-id="29bb9-497">验证控制器是否调用工作单元的 Commit 方法，以便在执行编辑时保存更改。</span><span class="sxs-lookup"><span data-stu-id="29bb9-497">Verify the controller invokes the unit of work’s Commit method to save changes when Edit executes.</span></span>

<span data-ttu-id="29bb9-498">交互测试通常需要较少的测试数据，因为我们不会在集合中闲逛和验证计数。</span><span class="sxs-lookup"><span data-stu-id="29bb9-498">Interaction testing often requires less test data, because we aren’t poking inside of collections and verifying counts.</span></span> <span data-ttu-id="29bb9-499">例如，如果我们知道详细信息操作将使用正确的值调用存储库的 FindById 方法，则操作可能会正常运行。</span><span class="sxs-lookup"><span data-stu-id="29bb9-499">For example, if we know the Details action invokes a repository’s FindById method with the correct value - then the action is probably behaving correctly.</span></span> <span data-ttu-id="29bb9-500">我们可以验证此行为，而无需设置任何要从 FindById 返回的测试数据。</span><span class="sxs-lookup"><span data-stu-id="29bb9-500">We can verify this behavior without setting up any test data to return from FindById.</span></span>
...
<span data-ttu-id="29bb9-501">上述测试装置中唯一需要的设置是由基类提供的设置。</span><span class="sxs-lookup"><span data-stu-id="29bb9-501">The only setup required in the above test fixture is the setup provided by the base class.</span></span> <span data-ttu-id="29bb9-502">当我们调用控制器操作时，Moq 将记录与 mock 存储库的交互。</span><span class="sxs-lookup"><span data-stu-id="29bb9-502">When we invoke the controller action, Moq will record the interactions with the mock repository.</span></span> <span data-ttu-id="29bb9-503">使用 Moq 的 Verify API，如果控制器使用正确的 ID 值调用 FindById，则可以询问 Moq。</span><span class="sxs-lookup"><span data-stu-id="29bb9-503">Using the Verify API of Moq, we can ask Moq if the controller invoked FindById with the proper ID value.</span></span> <span data-ttu-id="29bb9-504">如果控制器未调用方法，或调用方法时出现意外的参数值，则 Verify 方法将引发异常，并且测试将失败。</span><span class="sxs-lookup"><span data-stu-id="29bb9-504">If the controller did not invoke the method, or invoked the method with an unexpected parameter value, the Verify method will throw an exception and the test will fail.</span></span>

<span data-ttu-id="29bb9-505">下面是另一个示例，用于验证创建操作对当前工作单元调用 Commit。</span><span class="sxs-lookup"><span data-stu-id="29bb9-505">Here is another example to verify the Create action invokes Commit on the current unit of work.</span></span>
...
<span data-ttu-id="29bb9-506">交互测试有一个危险，就是指定交互。</span><span class="sxs-lookup"><span data-stu-id="29bb9-506">One danger with interaction testing is the tendency to over specify interactions.</span></span> <span data-ttu-id="29bb9-507">Mock 对象记录和验证每个与 mock 对象的交互的能力并不意味着测试应尝试验证每个交互。</span><span class="sxs-lookup"><span data-stu-id="29bb9-507">The ability of the mock object to record and verify every interaction with the mock object doesn’t mean the test should try to verify every interaction.</span></span> <span data-ttu-id="29bb9-508">某些交互是实现详细信息，只应验证满足当前测试 *所需* 的交互。</span><span class="sxs-lookup"><span data-stu-id="29bb9-508">Some interactions are implementation details and you should only verify the interactions *required* to satisfy the current test.</span></span>

<span data-ttu-id="29bb9-509">模拟或 fakes 之间的选择主要取决于你要测试的系统和个人 (或团队) 首选项。</span><span class="sxs-lookup"><span data-stu-id="29bb9-509">The choice between mocks or fakes largely depends on the system you are testing and your personal (or team) preferences.</span></span> <span data-ttu-id="29bb9-510">Mock 对象可以极大地减少实现测试双精度值所需的代码量，但并非每个人都能得心应手地进行编程和验证交互。</span><span class="sxs-lookup"><span data-stu-id="29bb9-510">Mock objects can drastically reduce the amount of code you need to implement test doubles, but not everyone is comfortable programming expectations and verifying interactions.</span></span>

## <a name="conclusions"></a><span data-ttu-id="29bb9-511">结论</span><span class="sxs-lookup"><span data-stu-id="29bb9-511">Conclusions</span></span>

<span data-ttu-id="29bb9-512">在本文中，我们演示了几种创建可测试代码的方法，同时使用 ADO.NET 实体框架来实现数据持久性。</span><span class="sxs-lookup"><span data-stu-id="29bb9-512">In this paper we’ve demonstrated several approaches to creating testable code while using the ADO.NET Entity Framework for data persistence.</span></span> <span data-ttu-id="29bb9-513">我们可以利用内置的抽象（如 IObjectSet &lt; t &gt; ），或创建自己的抽象（如 IRepository） &lt; &gt; 。</span><span class="sxs-lookup"><span data-stu-id="29bb9-513">We can leverage built in abstractions like IObjectSet&lt;T&gt;, or create our own abstractions like IRepository&lt;T&gt;.</span></span><span data-ttu-id="29bb9-514">在这两种情况下，ADO.NET 中的 POCO 支持实体框架4.0，使这些抽象的使用者能够维持永久性未知和高度可测试性。</span><span class="sxs-lookup"><span data-stu-id="29bb9-514">  In both cases, the POCO support in the ADO.NET Entity Framework 4.0 allows the consumers of these abstractions to remain persistent ignorant and highly testable.</span></span> <span data-ttu-id="29bb9-515">隐式延迟加载等附加 EF4 功能允许业务和应用程序服务代码工作，而无需担心关系数据存储的详细信息。</span><span class="sxs-lookup"><span data-stu-id="29bb9-515">Additional EF4 features like implicit lazy loading allows business and application service code to work without worrying about the details of a relational data store.</span></span> <span data-ttu-id="29bb9-516">最后，我们创建的抽象可以轻松地在单元测试中模拟或伪造，而且我们可以使用这些测试来实现快速运行、高度隔离和可靠的测试。</span><span class="sxs-lookup"><span data-stu-id="29bb9-516">Finally, the abstractions we create are easy to mock or fake inside of unit tests, and we can use these test doubles to achieve fast running, highly isolated, and reliable tests.</span></span>

### <a name="additional-resources"></a><span data-ttu-id="29bb9-517">其他资源</span><span class="sxs-lookup"><span data-stu-id="29bb9-517">Additional Resources</span></span>

-   <span data-ttu-id="29bb9-518">Robert，" [单责任原则](https://www.objectmentor.com/resources/articles/srp.pdf)"</span><span class="sxs-lookup"><span data-stu-id="29bb9-518">Robert C. Martin, “ [The Single Responsibility Principle](https://www.objectmentor.com/resources/articles/srp.pdf)”</span></span>
-   <span data-ttu-id="29bb9-519">圣马丁 Fowler，从*企业应用程序体系结构模式*的模式[目录](https://www.martinfowler.com/eaaCatalog/index.html)</span><span class="sxs-lookup"><span data-stu-id="29bb9-519">Martin Fowler, [Catalog of Patterns](https://www.martinfowler.com/eaaCatalog/index.html) from *Patterns of Enterprise Application Architecture*</span></span>
-   <span data-ttu-id="29bb9-520">Griffin Caprio，" [依赖关系注入](https://msdn.microsoft.com/magazine/cc163739.aspx)"</span><span class="sxs-lookup"><span data-stu-id="29bb9-520">Griffin Caprio, “ [Dependency Injection](https://msdn.microsoft.com/magazine/cc163739.aspx)”</span></span>
-   <span data-ttu-id="29bb9-521">数据可编程性博客，" [演练：测试驱动开发与实体框架 4.0](/archive/blogs/adonet/walkthrough-test-driven-development-with-the-entity-framework-4-0)"。</span><span class="sxs-lookup"><span data-stu-id="29bb9-521">Data Programmability Blog, “ [Walkthrough: Test Driven Development with the Entity Framework 4.0](/archive/blogs/adonet/walkthrough-test-driven-development-with-the-entity-framework-4-0)”.</span></span>
-   <span data-ttu-id="29bb9-522">数据可编程性博客，" [使用存储库和实体框架4.0 的工作单元模式](/archive/blogs/adonet/using-repository-and-unit-of-work-patterns-with-entity-framework-4-0)"</span><span class="sxs-lookup"><span data-stu-id="29bb9-522">Data Programmability Blog, “ [Using Repository and Unit of Work patterns with Entity Framework 4.0](/archive/blogs/adonet/using-repository-and-unit-of-work-patterns-with-entity-framework-4-0)”</span></span>
-   <span data-ttu-id="29bb9-523">Aaron Jensen，" [机器规格简介](http://codebetter.com/blogs/aaron.jensen/archive/2008/05/08/introducing-machine-specifications-or-mspec-for-short.aspx)"</span><span class="sxs-lookup"><span data-stu-id="29bb9-523">Aaron Jensen, “ [Introducing Machine Specifications](http://codebetter.com/blogs/aaron.jensen/archive/2008/05/08/introducing-machine-specifications-or-mspec-for-short.aspx)”</span></span>
-   <span data-ttu-id="29bb9-524">Eric 先生，" [BDD With MSTest](https://saintgimp.org/2009/01/20/bdd-with-mstest/)"</span><span class="sxs-lookup"><span data-stu-id="29bb9-524">Eric Lee, “ [BDD with MSTest](https://saintgimp.org/2009/01/20/bdd-with-mstest/)”</span></span>
-   <span data-ttu-id="29bb9-525">Eric Evans，" [域驱动设计](https://books.google.com/books?id=7dlaMs0SECsC&printsec=frontcover&dq=evans%20domain%20driven%20design&hl=en&ei=cHztS6C8KIaglAfA_dS1CA&sa=X&oi=book_result&ct=result&resnum=1&ved=0CCoQ6AEwAA)"</span><span class="sxs-lookup"><span data-stu-id="29bb9-525">Eric Evans, “ [Domain Driven Design](https://books.google.com/books?id=7dlaMs0SECsC&printsec=frontcover&dq=evans%20domain%20driven%20design&hl=en&ei=cHztS6C8KIaglAfA_dS1CA&sa=X&oi=book_result&ct=result&resnum=1&ved=0CCoQ6AEwAA)”</span></span>
-   <span data-ttu-id="29bb9-526">圣马丁 Fowler，" [模拟不是存根](https://martinfowler.com/articles/mocksArentStubs.html)"</span><span class="sxs-lookup"><span data-stu-id="29bb9-526">Martin Fowler, “ [Mocks Aren’t Stubs](https://martinfowler.com/articles/mocksArentStubs.html)”</span></span>
-   <span data-ttu-id="29bb9-527">圣马丁 Fowler，" [Test Double](https://martinfowler.com/bliki/TestDouble.html)"</span><span class="sxs-lookup"><span data-stu-id="29bb9-527">Martin Fowler, “ [Test Double](https://martinfowler.com/bliki/TestDouble.html)”</span></span>
-   [<span data-ttu-id="29bb9-528">Moq</span><span class="sxs-lookup"><span data-stu-id="29bb9-528">Moq</span></span>](https://code.google.com/p/moq/)

### <a name="biography"></a><span data-ttu-id="29bb9-529">事迹</span><span class="sxs-lookup"><span data-stu-id="29bb9-529">Biography</span></span>

<span data-ttu-id="29bb9-530">Scott Allen 是技术人员在 Pluralsight 和创始人 OdeToCode.com 的成员。</span><span class="sxs-lookup"><span data-stu-id="29bb9-530">Scott Allen is a member of the technical staff at Pluralsight and the founder of OdeToCode.com.</span></span> <span data-ttu-id="29bb9-531">在15年的商业软件开发中，Scott 已经处理了从8位嵌入式设备到高度可扩展的 ASP.NET web 应用程序的各种解决方案。</span><span class="sxs-lookup"><span data-stu-id="29bb9-531">In 15 years of commercial software development, Scott has worked on solutions for everything from 8-bit embedded devices to highly scalable ASP.NET web applications.</span></span> <span data-ttu-id="29bb9-532">你可以在 OdeToCode 或 Twitter 上的 Twitter 上联系 Scott [https://twitter.com/OdeToCode](https://twitter.com/OdeToCode) 。</span><span class="sxs-lookup"><span data-stu-id="29bb9-532">You can reach Scott on his blog at OdeToCode, or on Twitter at [https://twitter.com/OdeToCode](https://twitter.com/OdeToCode).</span></span>
