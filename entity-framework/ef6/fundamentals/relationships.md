...
在实体框架中，可以通过关联或关系将实体与其他实体相关。 每个关系都包含两个端，用于描述该关系中的两个实体的实体类型和类型（一、零或多个）的重数。 关系可由引用约束控制，该引用约束描述了关系中的哪端为 Principal Role 以及哪端为 Dependent Role。
...
- 通过使用 `ChangeRelationshipState` 方法更改两个实体对象之间的指定关系的状态。 此方法最常用于使用 N 层应用程序和*独立关联*（不能与外键关联一起使用）。 此外，若要使用此方法，必须将下拉到 `ObjectContext` ，如下面的示例中所示。  
