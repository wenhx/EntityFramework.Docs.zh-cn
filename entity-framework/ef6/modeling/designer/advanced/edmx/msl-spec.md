...
| **ReplTest1**      | 是          | 要与列值进行比较的值。 如果值不同，则该条件为 True。 否则，条件为 False。 <br/> 不能同时使用**IsNull**和**Value**属性。                                                                       |
| **Name**       | 是          | 概念模型实体属性的名称，其值用于计算条件。 <br/> 如果在 FunctionImportMapping 元素中使用**Condition**元素，则此属性不适用。                                                                           |
