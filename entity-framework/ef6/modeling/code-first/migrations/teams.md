...
迁移使用** \_ \_ MigrationsHistory**表来存储已应用到数据库的迁移。 如果有多个开发人员在尝试将同一数据库定向到目标时生成不同的迁移 (，因此共享** \_ \_ MigrationsHistory**表) 迁移将会变得很混乱。
...
自动迁移使你能够更新数据库架构以匹配当前模型，而无需生成代码文件 () 的基于代码的迁移。 如果你只使用了这些迁移，并且从未生成任何基于代码的迁移，则自动迁移将在团队环境中正常工作。 问题在于自动迁移是有限的，并且不处理多个操作–属性/列重命名、将数据移动到另一个表等。若要处理这些情况，您最终将生成基于代码的迁移 (并编辑在自动迁移处理的更改之间混合的基架代码) 。 这使得在两个开发人员签入迁移时，无法合并更改。
...
当前模型是从代码 (1) 计算得出的。 然后，模型会 (2) 来计算所需的数据库对象–因为这是第一次迁移模型不同只是使用空模型进行比较。 所需的更改会传递给代码生成器，以生成 (3) 所需的迁移代码，然后将其添加到 Visual Studio 解决方案 (4) 。

除了存储在主代码文件中的实际迁移代码外，迁移还会生成一些附加的代码隐藏文件。 这些文件是迁移使用的元数据，而不是你应该编辑的内容。 其中一项文件是资源文件 ( .resx) ，该文件包含在生成迁移时模型的快照。 下一步将介绍如何使用此方法。
...
与上一次一样，当前模型是从代码 (1) 计算得出的。 但是，这一次存在现有的迁移，因此从最新迁移 (2) 中检索上一个模型。 这两个模型 diffed，用于查找 (3) 所需的数据库更改，然后该进程将像以前一样完成。
...
> [!NOTE]
> 迁移以时间戳为前缀，因此我们的图形表示从 \# 开发人员1开始的 AddRating 迁移后，开发人员2的 AddReaders 迁移 \# 。 开发人员 \# 1 或 \# 2 是否首先生成了迁移，与团队中工作的问题或合并过程的过程没有什么不同，我们将在下一节中进行讨论。

![本地更改](~/ef6/media/localchanges.png)

这是开发人员1幸运的一天， \# 因为他们首先提交其更改。 由于其他人在同步其存储库后未签入，因此他们只需提交其更改而无需执行任何合并。

![提交更改](~/ef6/media/submit.png)

现在是时候开发人员2的时候了 \# 。 它们不太幸运。 由于其他人已在同步后提交了更改，因此他们将需要下拉更改并进行合并。 源代码管理系统可能会自动将更改合并到代码级别，因为它们非常简单。 \#在同步后，开发人员2的本地存储库的状态如下图所示。 

![从源控件拉取](~/ef6/media/pull.png)

在此阶段，开发人员 \# 2 可以运行 **更新数据库** ，该数据库将检测到尚未应用于 Developer 2 的数据库的新 **AddRating** 迁移 (\#) 并应用该迁移。 现在，" **评级** " 列将添加到 " **博客** " 表中，并且数据库与模型同步。

但有几个问题：

1.  尽管 " **更新-数据库** " 将应用 **AddRating** 迁移，但它还会引发警告： *无法更新数据库以匹配当前模型，因为存在挂起的更改，并且禁用了自动迁移 ...*
    问题在于，上一次迁移 (**AddReader**) 中存储的模型快照缺少**博客** (上的**分级**属性，因为它不是生成迁移) 时的模型的一部分。 Code First 检测到上一次迁移中的模型与当前模型不匹配，并引发警告。
...
3.  最后，我们希望运行 **添加迁移** 现在会生成一个空的迁移 (，因为没有要应用到数据库) 的更改。 但是，因为迁移会将当前模型与上一次迁移 (（缺少 **分级** 属性）的模型进行比较，) 它将实际基架另一次 **AddColumn** 调用以添加到 **分级** 列中。 当然，此迁移将在 **更新数据库** 过程中失败，因为 " **分级** " 列已经存在。
...
无论上次迁移的用户是谁，都可以使用此选项。 在本示例中，我们已关注开发人员 \# 2 正在处理合并，并且它们会生成上次迁移。 但是，如果开发人员 \# 1 生成了上一次迁移，则可以使用这些相同的步骤。 如果涉及多个迁移，则这些步骤也适用–我们只需查看两个步骤，使其保持简单。
...
**此方法仅在以下情况下可行：仅在本地代码库中存在最新的迁移，并且尚未提交到源代码管理 (例如，如果执行合并) 的用户生成了最后一个迁移 **。 编辑其他开发人员可能已应用于其开发数据库（甚至更糟）的迁移的元数据可能会导致意外的副作用。 在此过程中，我们将在本地数据库中回滚上一次迁移，并使用更新的元数据重新应用它。
...
5.  运行**添加迁移 &lt; 全名 \_ \_ （包括 \_ \_ \_ 上次 \_ 迁移的时间戳**） &gt; (在下面的示例中，这可能类似于**添加迁移 201311062215252 \_ AddReaders**) 。
    **_注意：_** *需要包含时间戳，以便迁移知道要编辑现有迁移，而不是新的基架。*
    这将更新上一次迁移的元数据以匹配当前模型。 当命令完成时，你将收到以下警告，但这正是你所希望的。 "*只有迁移" 201311062215252 AddReaders "的设计器代码 \_ 是重新基架的。若要重新基架整个迁移，请使用-Force 参数。 "*
6.  运行 **Update-Database** ，以使用更新的元数据重新应用最新的迁移。
7.  运行课程) 的单元测试后，继续开发或提交到源代码管理 (。
